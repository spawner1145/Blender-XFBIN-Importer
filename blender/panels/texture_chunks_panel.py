...
class NutTexturePropertyGroup(PropertyGroup):
    ...
    def init_data(self, nut_texture: NutTexture, tex_name, path = ''):
        ...
        # Populate the image before assigning the PointerProperty: its update
        # callback needs the packed DDS data to exist already.
        image = bpy.data.images.get(self.name)
        if image is None:
            image = bpy.data.images.new(tex_name, width=int(self.width), height=int(self.height))
