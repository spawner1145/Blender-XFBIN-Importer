...
def read_xfbin(file: Union[str, bytearray]) -> Xfbin:
    ...
    for br_page in br_xfbin.pages:
        ...
        for index in br_page.chunksDict:
            ...
            if type(chunk).__name__ in ("NuccChunkAnm", "NuccChunkCamera"):
                chunk = NuccChunk.create_from_nucc_type(*table.get_props_from_chunk_map(
                    table.chunkMaps[br_page.pageChunkIndices[index]]))
